...
#### Load preprocessed dataset (code in 2.2.Preprocessing_pipeline.Rmd)
...
Diagnosis and brain region seem to be balanced
```{r}
...
There are many more Male than Female samples, and there seems to be a small correlation between gender and diagnosis, with an underepresentation of male ASD samples
```{r}
cro(table_info$Diagnosis, list(table_info$Sex, total()))
```
<br>

The difference in age between diagnosis groups is not statistically significant, the only difference seems to be that ASD samples have a larger age dispersion
...
The first principal component separates the samples by diagnosis groups quite well, there is no recognisable pattern in the samples coloured by brain lobe
...
plot_data = pca$x %>% data.frame %>% dplyr::select(PC1:PC2) %>% 
            mutate('Sample_ID'= datExpr %>% colnames) %>% 
            left_join(datMeta, by='Sample_ID') %>% 
            dplyr::select('Sample_ID','PC1','PC2','Diagnosis','Brain_lobe') %>% 
...
p1 = plot_data %>% ggplot(aes(PC1, PC2, color=Diagnosis, shape=Diagnosis)) + geom_point(alpha = 0.8) +
              ...
              #ggtitle('Samples coloured by Diagnosis') + 
  coord_fixed() + theme_minimal() + theme(legend.position = 'bottom')

p2 = plot_data %>% ggplot(aes(PC1, PC2, color=Brain_lobe, shape=Brain_lobe)) + geom_point(alpha = 0.8) +
              xlab(paste0('PC1 (',round(100*summary(pca)$importance[2,1],1),'%)')) +
              ylab(paste0('PC2 (',round(100*summary(pca)$importance[2,2],1),'%)')) +
              #ggtitle('Samples coloured by Brain Lobe') + 
  coord_fixed() + labs(color = 'Brain Lobe', shape = 'Brain Lobe') + 
...
for(i in 1:length(perplexities)){
  ...
  plot_data = data.frame('ID'=colnames(datExpr), 'C1'=tsne$Y[,1], 'C2'=tsne$Y[,2]) %>%
              left_join(datMeta, by=c('ID'='Sample_ID')) %>%
...
This time, there does seem to be some small local clusters related to brain lobe
...
In the Gandal dataset, some perplexity values managed to capture the subject the samples belonged to, but it doesn’t seem to do it with this new dataset
...
Samples belonging to the ASD group have statistically significantly higher levels of expression than the Control group (this was unexpected and consistent with the results in the Gandal dataset)
...
Samples extracted from the frontal lobe have a slightly lower mean level of expression than the samples from the other lobes, but this difference is not statistically significant (this is consistent with the Gandal dataset)

```{r, fig.align='center'}

plot_data %>% ggplot(aes(Brain_lobe, Mean)) + 
...
There are `r sum(genes_info$padj<0.05, na.rm=TRUE)` DE genes using a threshold of LFC=0 (~`r round(mean(genes_info$padj<0.05, na.rm=TRUE)*100)`% of the total number of genes). As the threshold increases, the number of DE genes quickly decreases.
...
pcas_samps = pcas_samps %>% left_join(datMeta, by=c('ID'='Sample_ID')) %>%
...
- LFC thresholds from 0 to 0.2 seem to separate the samples by diagnosis better than the rest
...
# p2 = pcas_samps %>% filter(lfc == 0.05) %>% ggplot(aes(PC1, PC2, color=Diagnosis, shape=Diagnosis)) + 
#      geom_point(alpha=0.7) + theme_minimal() + ggtitle('LFC = 0.05') + theme(legend.position = 'none') + coord_fixed()
# p3 = pcas_samps %>% filter(lfc == 0.1) %>% ggplot(aes(PC1, PC2, color=Diagnosis, shape=Diagnosis)) + 
#      geom_point(alpha=0.7) + theme_minimal() + ggtitle('LFC = 0.1') + theme(legend.position = 'none') + coord_fixed()
# p4 = pcas_samps %>% filter(lfc == 0.2) %>% ggplot(aes(PC1, PC2, color=Diagnosis, shape=Diagnosis)) + 
#      geom_point(alpha=0.7) + theme_minimal() + ggtitle('LFC = 0.2') + coord_fixed()
