...
#setwd('~/PhD/Thesis/Dataset_Gupta/supportingScripts')
...
calc_performance_metrics = function(model, selected_set){
  ...
  predictions = model %>% predict(selected_set, type = 'prob') %>% 
                mutate(prob = SFARI, pred = prob>0.5, SFARI = selected_set$SFARI) %>% dplyr::select(-c(not_SFARI))
...
for(seed in seeds){
  ...
  if(seed == initial_seed){
    ...
    coefs = model_output$coefs %>% data.frame
  } else{
    ...
    coefs = cbind(coefs, model_output$coefs)
  }
...
ridge_coefs = data.frame('coef' = c('Intercept', colnames(dataset)[-ncol(dataset)]),
                                    'mean' = rowMeans(coefs), 'sd' = apply(coefs,1,sd))
ridge_pm_train = data.frame('metric' = rownames(pm_train), 'mean' = rowMeans(pm_train), 'sd' = apply(pm_train,1,sd))
ridge_pm_val = data.frame('metric' = rownames(pm_val), 'mean' = rowMeans(pm_val), 'sd' = apply(pm_val,1,sd))
ridge_pm_test = data.frame('metric' = rownames(pm_test), 'mean' = rowMeans(pm_test), 'sd' = apply(pm_val,1,sd))

# Calculate performance metrics of the final model
ridge_predictions = predictions %>% mutate(prob = prob/n, pred_count = pred, pred = prob>0.5) %>%
                    left_join(genes_info %>% dplyr::select(ID, hgnc_symbol), by = 'ID') %>% drop_na(pred)
ridge_pm_agg_preds = calc_pm_aggregated_predictions(ridge_predictions)

save(ridge_predictions, ridge_pm_agg_preds, ridge_pm_train, ridge_pm_val, ridge_pm_test, ridge_coefs,
     file = '../Data/Results/Ridge_regression.RData')
