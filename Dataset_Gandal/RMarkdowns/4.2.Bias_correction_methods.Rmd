...
The feature with the largest negative coefficient is the absolute value of the Gene Significance of the gene. This means that the magnitude of a gene's correlation to diagnosis decreases in a significant way its probability of being classified as a SFARI gene, which is consistent with previous findings that showed that SFARI genes have a lower dysregulation in gene expression than the rest of the genes.
...
- There is now a positive correlation between the coefficients and the cluster-diagnosis of the 

Together, these two plots show that the model is identifying the SFARI Genes using their similarity to clusters with a high enrichment with SFARI Genes as well as to clusters with a high cluster-diagnosis correlation.
...
As the previous models, the Weighting technique assigns statistically higher probabilities to SFARI Genes than to the rest of the genes, independently of having neuronal annotations or not, but this time, the probabilities assigned to each of the SFARI Scores are not visibly nor statistically different between Scores, which suggests the level of expression may no longer be playing an important role in the model's characterisation of the SFARI Genes.
...
Tthe Weighting technique successfully removes the positive relation between the mean level of expression of the genes and their probability assigned by the model, although this relation appears to have been replaced by a small negative linear trend.
...
As the percentiles between parenthesis show, these genes don't have particularly high Gene Significance values, with a mean value of 59.9 and a standard deviation of 14.6, but they do have larger cluster-diagnosis correlations, with an average percentile value of 62.1 and a standard deviation of 26.5, with genes such as SERPING1 belonging to a cluster with a correlation to ASD of 0.84 and ARHGAP20, with a correlation of -0.90. These two patterns found in the genes with the highest scores agree with the unusual patterns that had been previously found to characterise the SFARI Genes: a low ASD-related gene expression dysregulation but a strong association to dysregulated genes.
