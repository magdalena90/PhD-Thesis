...
```{r warning=FALSE, message=FALSE, fig.height56, fig.width=10}
...
There is a strong positive relation for all genes except for the ones with the lowest levels of expression. This suggests that the classifier is using the level of expression of a gene, or some confounder of it, as a factor when calculating its similarity to the SFARI genes.

```{r warning=FALSE, message=FALSE, fig.align='center'}

meanExpr = data.frame(ID=rownames(datExpr), meanExpr=rowMeans(datExpr))

plot_data = ridge_predictions %>% left_join(meanExpr, by='ID')
