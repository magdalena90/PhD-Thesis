...
best_power = datExpr %>% t %>% pickSoftThreshold(powerVector = c(seq(1,2.5,0.5),seq(2.6,3.5,0.1),4), RsquaredCut=0.8)
```
...
The clusters created by the Dynamic Tree algorithm are not that many so it seems like this merging step is not necessary
